...
4º Passo: ATENÇÃO, faça o mesmo processo do 2º e 3º passos desta etapa, habilitando agora a porta “443” para entrada. 
...
9º Passo: após reinício do serviço e aguardando o tempo de inicialização, que dependendo das configurações da sua instalação pode demorar alguns minutos, verifique se a instalação já pode ser acessada, digitando no navegador o caminho “localhost:443”. 
...
> Observação: se sua Instalação foi acessada, a porta 443 está devidamente configurada. 

10º Passo: acesse agora a sua Instalação, em um outro computador fora da rede local da Instalação, para testar se a porta 443 está corretamente configurada, da seguinte maneira “SEU-IP-PUBLICO:443” e a mesma mensagem acima deve surgir em seu navegador. 
...
- Os certificados no formato PEM criados pelo Certbot tem duração de 90 dias, porém, o próprio Certbot cria uma tarefa automática no Windows para realizar sua renovação em tempo oportuno, mas, para que isso ocorra, a porta pela qual o certificado foi gerado deve permanecer devidamente configurada. 
...
1 - Atenção, pois qualquer caractere equivocado ou espaço deixado no fim de cada linha pode fazer com que seu .bat não seja executado corretamente. 

2 - Onde é necessária alguma alteração, destacamos em vermelho. 

3 - Confira o caminho completo do seu certificado na pasta do Certbot: "C:\Certbot\live\esus.municipio.uf.gov.br". 

4 - Onde está "SUA-SENHA-PARA-O-CERTIFICADO", crie uma senha para seu certificado e anote-a para a próxima etapa de parametrização na instalação. 
...
> Observação: onde há no comando “SUA-SENHA-PARA-O-CERTIFICADO", substitua pela senha criada no 4º passo da Etapa 4 - Instalando o OpenSSL e criando o certificado P12. 

5º Passo: salve as alterações do arquivo. 
...
Após a habilitação do protocolo HTTPS, por motivos padrão de segurança do Android, é importante ressaltar que apenas os aplicativos da estratégia e-SUS APS instalados em Android versão 5.0 ou superior conseguirão realizar sincronização com a Instalação. 

 

FIM 

1ª versão. Atualizado em 25 de março de 2024. 